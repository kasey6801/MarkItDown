...
### Step 12: Windows hardening pass, v0.44.0 (2026-07-14)

**User prompt:**
> Can the Windows version be refactored so it will run on any Windows 11 system? It previously failed. Why?

**Claude actions:**
- Investigated the failure history: no runtime failure was recorded anywhere; the only logged failures were CI release-upload steps (fixed 2026-04-01). A code audit found five latent hazards for clean Windows 11 machines instead
- Hardened `app.py`: binds `127.0.0.1` on Windows (no Defender Firewall prompt) and keeps `0.0.0.0` on macOS; tries ports 5001-5010 instead of dying silently on a busy port; reuses an already-running instance by opening a browser tab to it; opens the browser when the server is actually ready instead of after a fixed delay; fatal startup errors now show a Windows error dialog and are logged to `%LOCALAPPDATA%\MarkItDown\markitdown.log`
- Gave `GET /heartbeat` an identification response used by instance detection and CI, without touching the watchdog (only POST resets the timer)
- Hardened both PyInstaller specs: explicitly bundle `azure.ai.documentintelligence`, `azure.identity`, `azure.core`, `pandas`, `numpy`, `markdownify`, `requests` (markitdown 0.0.2 imports all of them at module load; the azure namespace packages are a classic PyInstaller miss); build-time collection warnings are now printed instead of swallowed
- Gave the Windows EXE an icon (`MarkItDown_icon.ico`, generated by `make_icon.py`) and a Windows version resource (`windows_version_info.txt`), improving the SmartScreen dialog
- Added a CI smoke test that launches the built EXE on the Windows runner, loads the UI, converts a sample file over HTTP, and verifies clean shutdown before the artifact is uploaded or released
- Bumped the app version to v0.44.0 and released the Windows build as v0.44.0

### Step 13: Security fixes, v0.44.1 (2026-07-14)

**User prompt:**
> complete a QA and security review for markitdown. [...] Address the 3 must fix items.

**Claude actions:**
- Ran a QA and security review (three parallel agents: security audit, functional QA, GitHub release verification), then fixed the three must-fix findings
- `/convert-url` now validates the URL first (`_validate_public_url`): public `http`/`https` only, rejecting non-URL strings (which markitdown would read as local file paths, a local-file-read) and hosts that resolve to loopback/private/link-local/reserved addresses (SSRF). Rejected URLs return HTTP 400 with a clean message
...
| Problem | Fix |
|---|---|
| Port 5001 already in use | Handled automatically: an already-running MarkItDown gets a new browser tab; a foreign process on the port makes the app fall back to 5002-5010 |
| App fails to start (Windows) | An error dialog appears; details are in `%LOCALAPPDATA%\MarkItDown\markitdown.log` |
...
| Audio transcription fails | Install ffmpeg and ensure it is on PATH; also needs internet access |
